...
```powershell
.venv-tools\Scripts\python.exe .claude\skills\datagastro-recoleccion-polos\scripts\verify_stack.py

.venv-tools\Scripts\python.exe .claude\skills\datagastro-recoleccion-polos\scripts\crawl_public.py `
  --url https://example.com --allow-host example.com --output-dir .agent-tools\recoleccion\example

.venv-tools\Scripts\python.exe .claude\skills\datagastro-recoleccion-polos\scripts\browser_public.py `
  --url https://example.com --allow-host example.com

.venv-tools\Scripts\python.exe .claude\skills\datagastro-recoleccion-polos\scripts\osm_pois.py `
  --place "Comuna 1, Buenos Aires, Argentina" --dry-run

.venv-tools\Scripts\python.exe .claude\skills\datagastro-recoleccion-polos\scripts\duckdb_query.py `
  --sql "SELECT 1 AS ok"

.agent-tools\chromadb\.venv\Scripts\python.exe `
  .claude\skills\datagastro-recoleccion-polos\scripts\chroma_store.py smoke
```
