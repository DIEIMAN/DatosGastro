"""Genera la version final visual V3 del informe Cafecito DataGastro.

Cierra la version institucional de alto nivel: portada sobria, resumen
ejecutivo que no repite la portada, lectura territorial del publico, red
territorial de cafeterias participantes (red potencial de difusion, no
participacion fisica), cruce publico vs red de sedes y recomendaciones de
gestion. Lee el XLSX original en modo solo lectura y no lo modifica.
...
PDF_V3 = OUT_DIR / "INFORME_CAFECITO_DATAGASTRO_V3.pdf"
FINAL_PDF_V3 = FINAL_DIR / "INFORME_CAFECITO_DATAGASTRO_V3.pdf"
...
MAPA_SEDES = GRAF_DIR / "mapa_sedes_cafeterias_v3.png"
MAPA_COMBINADO = GRAF_DIR / "mapa_cafeterias_y_publico_cafecito.png"
PLAN_BARRAS = GRAF_DIR / "tipo_plan_barras_v3.png"
...
def draw_base_comunas(ax, data: dict, counts: dict[int, int] | None = None) -> float:
    """Dibuja el contorno comunal. Si hay counts, colorea coropletico. Devuelve max."""
    ...
    for feature in data["features"]:
        ...
        if counts is not None:
            ...
            color = cmap(0.16 + 0.70 * (count / max_count)) if count else "#f2f4f7"
        else:
            color = "#f3f6fa"
        for ring in rings(feature["geometry"]):
            xs = [p[0] for p in ring]
            ys = [p[1] for p in ring]
            ax.fill(xs, ys, facecolor=color, edgecolor="white", lw=1.1, zorder=1)
        ...
        if coords:
            ...
            if counts is not None:
                ...
                tcolor = INK if count < max_count * 0.6 else "white"
            else:
                label = f"C{comuna}"
                tcolor = GREY
            ax.text(cx, cy, label, ha="center", va="center", fontsize=7.0, color=tcolor, fontweight="bold", zorder=3)
    return max_count
...
def draw_mapa_sedes(sedes: list[dict]) -> bool:
    ...
    draw_base_comunas(ax, data, counts=None)
    xs = [float(s["longitud"]) for s in sedes]
    ys = [float(s["latitud"]) for s in sedes]
    ax.scatter(xs, ys, s=46, color=COFFEE, edgecolor="white", lw=0.9, zorder=4, alpha=0.92)
    ax.set_title("Red de sedes públicas conocidas de cafeterías participantes", loc="left",
                 color=INK, fontsize=13, fontweight="bold", pad=12)
    ax.text(0, 1.01, "Sedes geocodificadas en CABA. Red potencial de difusión, no asistencia ni participación física.",
            transform=ax.transAxes, fontsize=8.6, color=GREY)
    ax.set_aspect("equal")
    ax.axis("off")
    handles = [Line2D([0], [0], marker="o", color="none", markerfacecolor=COFFEE, markeredgecolor="white", markersize=9, label="Sede de cafetería (red potencial)")]
    ax.legend(handles=handles, loc="lower left", frameon=False, fontsize=8.4)
    fig.tight_layout(rect=[0, 0.02, 1, 0.96])
    GRAF_DIR.mkdir(parents=True, exist_ok=True)
    fig.savefig(MAPA_SEDES, dpi=190, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    return True
...
def draw_mapa_combinado(sedes: list[dict], ranking_rows: list[dict]) -> bool:
    ...
    fig, ax = plt.subplots(figsize=(7.9, 8.4))
    max_count = draw_base_comunas(ax, data, counts=counts)
    xs = [float(s["longitud"]) for s in sedes]
    ys = [float(s["latitud"]) for s in sedes]
    ax.scatter(xs, ys, s=42, color=COFFEE, edgecolor="white", lw=0.9, zorder=4, alpha=0.95)
    ax.set_title("Público encuestado por comuna y red de sedes de cafeterías", loc="left",
                 color=INK, fontsize=13, fontweight="bold", pad=12)
    ax.text(0, 1.01, "Coropleta: respuestas agregadas por comuna. Puntos: sedes de cafeterías (red potencial de difusión).",
            transform=ax.transAxes, fontsize=8.4, color=GREY)
    ax.set_aspect("equal")
    ax.axis("off")
    handles = [
        Line2D([0], [0], marker="s", color="none", markerfacecolor=plt.get_cmap("Blues")(0.7), markersize=10, label="Más respuestas del público (por comuna)"),
        Line2D([0], [0], marker="o", color="none", markerfacecolor=COFFEE, markeredgecolor="white", markersize=9, label="Sede de cafetería (red potencial)"),
    ]
    ax.legend(handles=handles, loc="lower left", frameon=False, fontsize=8.2)
    fig.text(0.02, 0.02,
             "No se afirma causalidad entre la ubicación de las sedes y la procedencia del público.",
             color=GREY, fontsize=8)
    fig.tight_layout(rect=[0, 0.04, 1, 0.95])
    GRAF_DIR.mkdir(parents=True, exist_ok=True)
    fig.savefig(MAPA_COMBINADO, dpi=190, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    return True
...
NETWORK_NOTE = (
    ...
    "participantes o vinculadas al evento. No implica que todas esas sedes hayan "
...
def cover_page(pdf: PdfPages) -> None:
    ...
    fig.text(0.065, 0.728, "Lectura de público, territorio y red de cafeterías participantes",
...
def summary_page(pdf: PdfPages, p: dict, page_no: int) -> None:
    ...
    add_box(fig, 0.515, 0.420, 0.42, 0.245, "Qué oportunidad abre", [
        ...
        "Una red de 39 sedes de cafeterías en CABA que hoy no aparece como canal de llegada: potencial de difusión física aún sin activar.",
...
def profile_page(pdf: PdfPages, stats: dict, p: dict, page_no: int) -> None:
    ...
    stacked(fig, [0.560, 0.730, 0.360, 0.080], stats["genero"]["items"], stats["genero"]["base"],
    ...
    add_box(fig, 0.515, 0.255, 0.42, 0.180, "Cuidado metodológico",
            "La procedencia se publica a nivel macro (CABA / no CABA) y, más adelante, agregada por comuna. El barrio/localidad libre queda fuera de la lectura pública por privacidad y calidad del dato.",
...
def public_territory_page(pdf: PdfPages, ranking_rows: list[dict], page_no: int) -> None:
    ...
    add_image(fig, MAPA_COMUNAL, [0.055, 0.235, 0.56, 0.560])
    ...
    add_box(fig, 0.630, 0.560, 0.305, 0.235, "Cómo leer el mapa",
    ...
    add_box(fig, 0.630, 0.335, 0.305, 0.205, "Cuidado metodológico",
    ...
    add_box(fig, 0.065, 0.110, 0.87, 0.105, "Lectura ejecutiva",
...
def network_page(pdf: PdfPages, sedes_comuna: list[dict], page_no: int) -> None:
    fig = page()
    header(fig, page_no, "Red territorial de cafeterías participantes",
           "Sedes públicas conocidas en CABA como red potencial de difusión.")
    add_image(fig, MAPA_SEDES, [0.055, 0.250, 0.56, 0.545])
    add_box(fig, 0.630, 0.600, 0.305, 0.195, "La red en números", [
        "14 marcas analizadas.",
        "39 sedes mapeables en CABA.",
        "2 marcas quedan pendientes de validación.",
    ], face=WHITE, edge=BLUE, bullet=True, body_size=9.0)
    top = [r for r in sedes_comuna if str(r["comuna"]).isdigit()][:6]
    top_txt = ", ".join(f"Comuna {r['comuna']} ({r['cantidad_sedes']})" for r in top)
    add_box(fig, 0.630, 0.400, 0.305, 0.180, "Comunas con más sedes",
            f"{top_txt}. Palermo, Belgrano y el corredor central concentran la mayor densidad de sedes.",
            face=SOFT_BLUE, edge=BLUE, body_size=8.6)
    add_box(fig, 0.065, 0.110, 0.87, 0.130, "Cómo leer esta red", NETWORK_NOTE,
            face=SOFT_ORANGE, edge=ORANGE, body_size=9.0)
    footer(fig)
    pdf.savefig(fig)
    plt.close(fig)
...
def public_vs_network_page(pdf: PdfPages, page_no: int) -> None:
    ...
    add_image(fig, MAPA_COMBINADO, [0.055, 0.240, 0.56, 0.560])
    add_box(fig, 0.630, 0.605, 0.305, 0.195, "Coincidencias", [
    ...
    ], face=SOFT_BLUE, edge=BLUE, bullet=True, body_size=8.6)
    add_box(fig, 0.630, 0.400, 0.305, 0.185, "Oportunidad", [
        "La red cubre comunas donde hoy hay poco público registrado.",
        "Son zonas para ampliar la difusión física en próximas ediciones.",
    ], face=SOFT_GREEN, edge=GREEN, bullet=True, body_size=8.6)
    add_box(fig, 0.065, 0.110, 0.87, 0.115, "Cuidado metodológico",
...
def loyalty_page(pdf: PdfPages, stats: dict, p: dict, records: list[dict], page_no: int) -> None:
    ...
    rows = []
    for label in ["No, ya participé de otros eventos", "Si, es la primera vez"]:
        row = table.get(label, Counter())
        if row:
            disp = "Recurrentes" if label.startswith("No") else "Primera vez"
            rows.append((disp, str(row.get("Si, acepto", 0)), str(sum(row.values()))))
    mini_table(fig, 0.090, 0.430, 0.760, 0.095, rows, "Aceptan contacto / total, por vínculo previo")
...
def motivations_page(pdf: PdfPages, stats: dict, p: dict, page_no: int) -> None:
    ...
    hbar(fig, [0.085, 0.560, 0.40, 0.250], stats["que_intereso"]["items"], stats["que_intereso"]["base"],
         "Qué fue lo que más interesó", color=COFFEE,
    ...
    hbar(fig, [0.560, 0.470, 0.375, 0.340], stats["eventos_futuros"]["items"], stats["eventos_futuros"]["base"],
         "Intereses para próximas ediciones", color=BLUE,
         accents={"Vino", "Pasteleria", "Food trucks", "Café", "Ferias y mercados"}, limit=8, multi=True)
    add_box(fig, 0.065, 0.295, 0.42, 0.140, "Qué interesó del evento",
...
def conclusions_page(pdf: PdfPages, p: dict, page_no: int) -> None:
    ...
    cierre = [
        ...
        "La red de 39 sedes de cafeterías ofrece una infraestructura territorial de difusión todavía sin activar, que cubre comunas más allá del entorno inmediato del evento.",
    ]
    yy = 0.778
    for t in cierre:
...
def annex_page(pdf: PdfPages, page_no: int) -> None:
    ...
    add_box(fig, 0.515, 0.380, 0.42, 0.205, "Red de cafeterías", [
        "Sedes públicas conocidas en CABA de marcas participantes o vinculadas.",
...
def write_pdf(records: list[dict], stats: dict, ranking_rows: list[dict], sedes_comuna: list[dict]) -> int:
    ...
    with PdfPages(PDF_V3) as pdf:
        ...
        annex_page(pdf, 12)
    shutil.copy2(PDF_V3, FINAL_PDF_V3)
    return 12
...
def write_markdown_report(records: list[dict], stats: dict, ranking_rows: list[dict], sedes_comuna: list[dict]) -> None:
    ...
    lines = [
        ...
        "- La red de **39 sedes** de cafeterías participantes ofrece una infraestructura de difusión territorial todavía sin activar.",
        "",
        ...
        "",
        "## 5. Red territorial de cafeterías participantes",
        "",
...
def update_readme() -> None:
    ...
    block = """## Versión DataGastro V3 (final)
...
```
"""
    pattern = r"(?ms)^## Versión DataGastro V3 \(final\)\n.*?(?=^## |\Z)"
...
def ensure_analysis_outputs() -> None:
    needed = [RANKING_COMUNAL, MAPA_COMUNAL]
...
def main() -> None:
    ...
    print("=" * 72)
    print("Informe Cafecito DataGastro V3 (final)")
    ...
    print(f"  PDF: {PDF_V3}")
    print(f"  Copia final: {FINAL_PDF_V3}")
    print("=" * 72)
