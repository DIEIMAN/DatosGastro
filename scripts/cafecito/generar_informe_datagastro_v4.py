"""Genera la version final visual V4 del informe Cafecito DataGastro.

Iteracion quirurgica sobre la maqueta V3: corrige titulos/subtitulos de
graficos y mapas, agranda y mejora los mapas comunal/sedes/combinado,
precisa la terminologia sobre sedes (sedes conocidas / red potencial, no
participacion fisica), estandariza "TACC", muestra la categoria minoritaria
de genero y cierra el cruce de aceptacion de contacto. Lee el XLSX original
en modo solo lectura y no lo modifica.
...
PDF_V4 = OUT_DIR / "INFORME_CAFECITO_DATAGASTRO_V4.pdf"
FINAL_PDF_V4 = FINAL_DIR / "INFORME_CAFECITO_DATAGASTRO_V4.pdf"
...
MAPA_COMUNAL = GRAF_DIR / "mapa_comunal_publico_v4.png"
MAPA_SEDES = GRAF_DIR / "mapa_sedes_cafeterias_v4.png"
MAPA_COMBINADO = GRAF_DIR / "mapa_cafeterias_y_publico_v4.png"
PLAN_BARRAS = GRAF_DIR / "tipo_plan_barras_v4.png"
...
def draw_mapa_comunal_publico(ranking_rows: list[dict]) -> bool:
    ...
    _title_block(ax, "Público que respondió, por comuna",
                 "Respuestas agregadas por comuna (CABA). No se publican barrios ni respuestas individuales.")
    ax.set_aspect("equal")
    ax.axis("off")
    _scale_legend(fig, ax, max_count)
    fig.subplots_adjust(left=0.02, right=0.98, top=0.92, bottom=0.13)
...
def draw_mapa_sedes(sedes: list[dict]) -> bool:
    ...
    _title_block(ax, "Red territorial de cafeterías vinculadas",
                 "Sedes públicas conocidas en CABA de marcas participantes o vinculadas. Red potencial de difusión.")
    ax.set_aspect("equal")
    ax.axis("off")
...
def draw_mapa_combinado(sedes: list[dict], ranking_rows: list[dict]) -> bool:
    ...
    _title_block(ax, "Público por comuna y red de cafeterías",
                 "Dos capas independientes: color = público agregado por comuna; puntos = sedes conocidas (red potencial).")
    ax.set_aspect("equal")
    ax.axis("off")
    handles = [
    ...
    ax.legend(handles=handles, loc="lower left", frameon=False, fontsize=8.8)
    _scale_legend(fig, ax, max_count)
    fig.text(0.50, 0.085, "No se afirma causalidad entre la ubicación de las sedes y la procedencia del público.",
             color=GREY, fontsize=8.2, ha="left")
    fig.subplots_adjust(left=0.02, right=0.98, top=0.92, bottom=0.13)
    GRAF_DIR.mkdir(parents=True, exist_ok=True)
...
def profile_page(pdf: PdfPages, stats: dict, p: dict, page_no: int) -> None:
    ...
    stacked(fig, [0.560, 0.745, 0.360, 0.072], stats["genero"]["items"], stats["genero"]["base"],
            "Género declarado", [INK, ORANGE, GREEN])
    gen = items_dict(stats, "genero")
    nb = gen.get("No binario", 0)
    fig.text(0.560, 0.700, f"Incluye una categoría minoritaria (No binario: {nb}). Porcentajes redondeados.",
             color=GREY, fontsize=7.6, va="top")
    caba = p["caba"]
    no_caba = p["n"] - caba
...
def public_territory_page(pdf: PdfPages, ranking_rows: list[dict], page_no: int) -> None:
    ...
    add_image(fig, MAPA_COMUNAL, [0.045, 0.300, 0.66, 0.560])
    ...
    add_box(fig, 0.715, 0.585, 0.225, 0.275, "Cómo leer el mapa",
    ...
    add_box(fig, 0.715, 0.320, 0.225, 0.245, "Cuidado metodológico",
    ...
    add_box(fig, 0.065, 0.110, 0.87, 0.150, "Lectura ejecutiva",
...
def network_page(pdf: PdfPages, sedes_comuna: list[dict], page_no: int) -> None:
    ...
    add_image(fig, MAPA_SEDES, [0.045, 0.300, 0.66, 0.560])
    add_box(fig, 0.715, 0.640, 0.225, 0.220, "La red en números", [
    ...
    add_box(fig, 0.715, 0.395, 0.225, 0.225, "Comunas con más sedes",
            f"{top_txt}. Palermo, Belgrano y el corredor central concentran la mayor densidad.",
            face=SOFT_BLUE, edge=BLUE, body_size=8.2)
    add_box(fig, 0.065, 0.110, 0.87, 0.150, "Cómo leer esta red", NETWORK_NOTE,
            face=SOFT_ORANGE, edge=ORANGE, body_size=9.0)
    footer(fig)
    pdf.savefig(fig)
    plt.close(fig)
...
def public_vs_network_page(pdf: PdfPages, page_no: int) -> None:
    ...
    add_image(fig, MAPA_COMBINADO, [0.045, 0.300, 0.66, 0.560])
    add_box(fig, 0.715, 0.640, 0.225, 0.220, "Coincidencias", [
    ...
    add_box(fig, 0.715, 0.395, 0.225, 0.225, "Oportunidad", [
        "La red cubre comunas con poco público registrado.",
        "Son zonas para ampliar la difusión física en próximas ediciones.",
    ], face=SOFT_GREEN, edge=GREEN, bullet=True, body_size=8.2)
    add_box(fig, 0.065, 0.110, 0.87, 0.150, "Cuidado metodológico",
...
def motivations_page(pdf: PdfPages, stats: dict, p: dict, page_no: int) -> None:
    ...
    hbar(fig, [0.560, 0.470, 0.375, 0.340], stats["eventos_futuros"]["items"], stats["eventos_futuros"]["base"],
         "Intereses para próximas ediciones (menciones)", color=BLUE,
...
def write_pdf(records: list[dict], stats: dict, ranking_rows: list[dict], sedes_comuna: list[dict]) -> int:
    ...
    with PdfPages(PDF_V4) as pdf:
        ...
        annex_page(pdf, 12)
    shutil.copy2(PDF_V4, FINAL_PDF_V4)
    return 12
...
def update_readme() -> None:
    ...
    block_v4 = """## Versión DataGastro V4 (final)
...
```
"""
    pattern_v4 = r"(?ms)^## Versión DataGastro V4 \(final\)\n.*?(?=^## |\Z)"
    if re.search(pattern_v4, text):
        text = re.sub(pattern_v4, block_v4.rstrip() + "\n\n", text)
    else:
        text = text.rstrip() + "\n\n" + block_v4
...
def main() -> None:
    ...
    print("=" * 72)
    print("Informe Cafecito DataGastro V4 (final)")
    ...
    print(f"  PDF: {PDF_V4}")
    print(f"  Copia final: {FINAL_PDF_V4}")
    print("=" * 72)
