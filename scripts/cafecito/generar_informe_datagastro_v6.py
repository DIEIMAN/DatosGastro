"""Genera la version final visual V6 del informe Cafecito DataGastro.

Correccion minima (microcorreccion) sobre la maqueta V5: ajusta
terminologia de sedes (solo "marcas/cafeterias vinculadas") en 
anexo metodologico, apaga el color de la categoria minoritaria 
de genero para mayor consistencia visual.
...
PDF_V6 = OUT_DIR / "INFORME_CAFECITO_DATAGASTRO_V6.pdf"
FINAL_PDF_V6 = FINAL_DIR / "INFORME_CAFECITO_DATAGASTRO_V6.pdf"
...
MAPA_COMUNAL = GRAF_DIR / "mapa_comunal_publico_v6.png"
MAPA_SEDES = GRAF_DIR / "mapa_sedes_cafeterias_v6.png"
MAPA_COMBINADO = GRAF_DIR / "mapa_cafeterias_y_publico_v6.png"
PLAN_BARRAS = GRAF_DIR / "tipo_plan_barras_v6.png"
...
def write_pdf(records: list[dict], stats: dict, ranking_rows: list[dict], sedes_comuna: list[dict]) -> int:
    ...
    with PdfPages(PDF_V6) as pdf:
        ...
        annex_page(pdf, 12)
    shutil.copy2(PDF_V6, FINAL_PDF_V6)
    return 12
...
def update_readme() -> None:
    ...
    block_v6 = """## Versión DataGastro V6 (final)
...
"""
    pattern_v6 = r"(?ms)^## Versión DataGastro V6 \(final\)\n.*?(?=^## |\Z)"
    if re.search(pattern_v6, text):
        text = re.sub(pattern_v6, block_v6.rstrip() + "\n\n", text)
    else:
        text = text.rstrip() + "\n\n" + block_v6
...
def main() -> None:
    ...
    print("=" * 72)
    print("Informe Cafecito DataGastro V6 (final)")
    ...
    print(f"  PDF: {PDF_V6}")
    print(f"  Copia final: {FINAL_PDF_V6}")
    print("=" * 72)
