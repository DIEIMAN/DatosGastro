"""Genera el informe Cafecito · REVISIÓN 4 (DGDGAS).

Cuarta revisión sobre la revisión 3. Corrección puntual de residencia:

  - Separa las DOS preguntas de residencia que estaban fusionadas:
      A) ¿Dónde vivís actualmente?  (cerrada, opción única) -> zona agregada.
      B) Indicanos tu barrio o localidad  (abierta) -> barrios/localidades.
    Se corrige en la página de preguntas (2) y en la sección de residencia (4),
    con sus dos subsecciones (4.1 zona agregada, 4.2 barrios/localidades) y la
    fuente de cada una.
  - Quita el texto metodológico largo del top de barrios (página 6); deja, si
    acaso, una aclaración mínima.
  - Mantiene el top de barrios/localidades (agregado, sin respuestas
    individuales) y las bases de gráfico fuera de las cajas de lectura.

El resto del informe se mantiene como en la revisión 3 (marca DGDGAS, índice con
páginas, secciones numeradas, explicación de multi-respuesta con menciones,
recomendaciones en potencial, anexo de cafeterías).

Reglas del PDF público: sin rutas, scripts, hashes, versiones internas ni datos
personales. El público se muestra agregado.

NO sobrescribe el FINAL, el EDITABLE_TEST ni las revisiones 1, 2, 2·formato o 3.

Salidas:
  - outputs/cafecito/INFORME_CAFECITO_DGDGAS_REVISION_4.pdf
  - Cafesito/final/INFORME_CAFECITO_DGDGAS_REVISION_4.pdf

Uso:
    python scripts/cafecito/generar_informe_cafecito_revision_4.py
"""
...
CONTENIDO_YAML = DOCS_DIR / "contenido_editable_informe_cafecito_revision_4.yaml"

PDF_REV = OUT_DIR / "INFORME_CAFECITO_DGDGAS_REVISION_4.pdf"
FINAL_PDF_REV = FINAL_DIR / "INFORME_CAFECITO_DGDGAS_REVISION_4.pdf"

MAPA_SEDES = final.MAPA_SEDES
MAPA_COMBINADO = final.MAPA_COMBINADO

KICKER = "DGDGAS"
FOOTER_REV4 = "DGDGAS · Cafecito BA en tu barrio · Resultados de encuestas"

# Lista de preguntas del formulario, con las DOS preguntas de residencia
# SEPARADAS (en la revisión 1 venían fusionadas como una sola). El texto y los
# objetivos provienen del formulario real; no se inventan preguntas.
PREGUNTAS_FORMULARIO = [
    ...
    {
        "pregunta": "¿Dónde vivís actualmente?",
        "tipo": "Cerrada, opción única",
        "objetivo": "Conocer la zona de residencia declarada por las personas encuestadas.",
    },
    {
        "pregunta": "Indicanos tu barrio o localidad",
        "tipo": "Abierta",
        "objetivo": "Identificar, de forma agregada, los barrios o localidades mencionados por las personas encuestadas.",
    },
    {
...
def footer(fig):
    ...
    fig.text(0.065, 0.035, FOOTER_REV4, color=GREY, fontsize=7.8, va="center")
...
def mini_top_table(fig, x, y_top, w, filas, otras, total, *, titulo, fuente="", nota=""):
    """Tabla compacta del top de barrios/localidades: nombre · % · (n).

    Anclada arriba en y_top; devuelve el y del borde inferior. La `fuente`
    (breve) va debajo del título; la `nota` solo se dibuja si se pasa (en la
    revisión 4 se quitó el texto metodológico largo)."""
    fig.text(x, y_top, clean_text(titulo), color=INK, fontsize=10.5, fontweight="bold", va="top")
    y = y_top - 0.024
    if fuente:
        fig.text(x, y, clean_text(fuente), color=GREY, fontsize=7.6, va="top")
        y -= 0.020
    row_h = 0.030
...
def general_data_page(pdf, c, inst, ev, filas, con_ts, n_total, page_no):
    ...
    ficha = [
        ...
        ("Lugar", ev["lugar"]),
        ("Dirección", ev["direccion"]),
        ("Fechas", [ev["fecha_sabado"], ev["fecha_domingo"]]),
    ...
    for k, v in ficha:
        ...
        for valor in valores:
            for line in wrap_text(valor, 70):
...
def questions_page(pdf, c, page_no):
    """Las preguntas del formulario en una sola hoja. Con la residencia separada
    son 10 preguntas; se compacta levemente el interlineado para que entren
    prolijas. Si la nota al pie no entra, se omite (las preguntas tienen
    prioridad)."""
    fig = page()
    header(fig, page_no, c["titulo"], c["subtitulo"])
    descriptive_lines(fig, 0.065, 0.845, 0.87, c["intro"], fontsize=8.6, color="#333333")
    x, w = 0.065, 0.87
    col_preg_w = 0.40
    gap = 0.02
    x_obj = x + col_preg_w + gap
    nota_lines = wrap_text(c["nota_pie"], 150) if c.get("nota_pie") else []
    y_floor = 0.100 + 0.0125 * len(nota_lines) + 0.008
    y = 0.808
    row_gap = 0.0085
    line_h = 0.0142
    for i, q in enumerate(PREGUNTAS_FORMULARIO, 1):
        ...
        for line in obj_lines:
            ...
            ty -= line_h
        y = y_box - row_gap
    if nota_lines and (y - 0.008) > y_floor:
        ny = 0.112 + 0.0125 * (len(nota_lines) - 1)
        for line in nota_lines:
            fig.text(0.065, ny, clean_text(line), color=GREY, fontsize=7.2, va="top")
            ny -= 0.0125
    footer(fig)
    pdf.savefig(fig)
    final.plt.close(fig)
...
def _residence_question_block(fig, x, y_top, w, c):
    """Bloque introductorio con las DOS preguntas de residencia, separadas:
    una cerrada (zona) y una abierta (barrio/localidad). No las fusiona."""
    pad = 0.013
    wrap_w = max(20, int((w - 2 * pad) * 100))
    line_h = 0.0165
    bloques = [
        (c["preguntas_titulo"], "head"),
        (c["pregunta_a"], "q"),
        (c["tipo_a"], "t"),
        (c["pregunta_b"], "q"),
        (c["tipo_b"], "t"),
    ...
    yy = y_top - 0.017
    estilos = {
        "head": dict(color=INK, weight="bold"),
        "q": dict(color=INK, weight="bold"),
        "t": dict(color=BLUE, weight="normal"),
        "o": dict(color="#333333", weight="normal"),
    }
    for txt, kind in bloques:
        st = estilos[kind]
        for line in wrap_text(txt, wrap_w):
            fig.text(x + pad, yy, line, color=st["color"], fontsize=8.6,
                     fontweight=st["weight"], va="top")
            yy -= line_h
    return y


def residence_page(pdf, c, stats, barrios, page_no):
    """Residencia: dos preguntas separadas + 4.1 zona agregada (izq) +
    4.2 barrios/localidades (der) + lectura."""
    fig = page()
    header(fig, page_no, c["titulo"], c["subtitulo"])
    b = _residence_question_block(fig, 0.065, 0.792, 0.87, c)
    top = b - 0.030
    # Columna izquierda (4.1): gráfico macro por zona (pregunta cerrada).
    fig.text(0.065, top, clean_text(c["macro_titulo"]), color=INK, fontsize=10.5, fontweight="bold", va="top")
    fig.text(0.065, top - 0.020, clean_text(c["macro_fuente"]), color=GREY, fontsize=7.6, va="top")
    hbar_pct(fig, [0.130, top - 0.320, 0.34, 0.250], stats["donde_vivis"]["items"], stats["donde_vivis"]["base"],
             "Por zona (CABA / GBA / PBA)", label_wrap=16)
    # Columna derecha (4.2): top barrios/localidades (pregunta abierta).
    if barrios:
        filas, total, otras = barrios
        mini_top_table(fig, 0.520, top, 0.415, filas, otras, total,
                       titulo=c["barrios_titulo"], fuente=c["barrios_fuente"], nota=c.get("barrios_nota", ""))
    else:
...
def motivaciones_page(pdf, c, stats, page_no):
    ...
    top = b - 0.040
    # Corrección quirúrgica de formato: se corre el área de barras un poco a la
    # derecha y se reduce el ancho de ajuste de los labels (label_wrap) solo en
    # este gráfico, para que "Conocer propuestas nuevas" se envuelva en dos
    # líneas y no se meta sobre el área de barras.
    hbar_pct(fig, [0.265, top - 0.290, 0.47, 0.290], stats["que_intereso"]["items"], stats["que_intereso"]["base"],
             "Qué fue lo que más le interesó", color=final.COFFEE,
             accents={"Probar café / cafeterías"}, label_wrap=18)
    lectura_box(fig, 0.065, 0.150, 0.87, c["lectura"])
    footer(fig)
    pdf.savefig(fig)
    final.plt.close(fig)
...
def interests_page(pdf, c, stats, page_no):
    ...
    b = question_block(fig, 0.065, 0.790, 0.87, c["pregunta"], c["tipo"], c["observa"])
    ny = descriptive_lines(fig, 0.065, b - 0.012, 0.87, c["nota_multi"], fontsize=8.4, color="#444444")
    # Caja de lectura primero (anclada abajo). El gráfico se apoya bien por
    # encima y su nota de "Base" queda con AIRE respecto de la caja: la base se
    # dibuja 0.020 por debajo del rect, así que dejamos 0.055 de margen para que
    # no quede pegada ni dentro de la caja de lectura.
    lec_top = lectura_box(fig, 0.065, 0.135, 0.87, c["lectura"])
    graf_top = ny - 0.028
    graf_bottom = lec_top + 0.055
...
def main():
    ...
    if not final.MAPA_COMUNAL.exists():
        final.draw_mapa_comunal_publico(ranking_rows)
    if not MAPA_SEDES.exists():
        final.draw_mapa_sedes(sedes_mapa)
    if not MAPA_COMBINADO.exists():
    ...
    print("=" * 72)
    print("Informe Cafecito DGDGAS · REVISIÓN 4")
    print(f"  Contenido editable: {CONTENIDO_YAML.relative_to(REPO_ROOT)}")
    print(f"  Respuestas: {len(records)}  ·  con marca temporal: {con_ts}")
    print(f"  Preguntas del formulario listadas: {len(PREGUNTAS_FORMULARIO)} (residencia separada en 2)")
    if barrios:
    ...
    print("  (FINAL, EDITABLE_TEST y revisiones 1/2/2-formato/3 NO fueron modificados.)")
    print("=" * 72)
