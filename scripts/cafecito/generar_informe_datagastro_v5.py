"""Genera la version final visual V5 del informe Cafecito DataGastro.

Correccion final de layout sobre la maqueta V4: quita titulos internos
de los mapas PNG (se usan solo los titulos de pagina del PDF), corrige
notas de genero truncadas, acorta titulos de graficos que se cortaban,
y cambia la numeracion de pagina a "Pag. X".
...
PDF_V5 = OUT_DIR / "INFORME_CAFECITO_DATAGASTRO_V5.pdf"
FINAL_PDF_V5 = FINAL_DIR / "INFORME_CAFECITO_DATAGASTRO_V5.pdf"
...
MAPA_COMUNAL = GRAF_DIR / "mapa_comunal_publico_v5.png"
MAPA_SEDES = GRAF_DIR / "mapa_sedes_cafeterias_v5.png"
MAPA_COMBINADO = GRAF_DIR / "mapa_cafeterias_y_publico_v5.png"
PLAN_BARRAS = GRAF_DIR / "tipo_plan_barras_v5.png"
...
def write_pdf(records: list[dict], stats: dict, ranking_rows: list[dict], sedes_comuna: list[dict]) -> int:
    ...
    with PdfPages(PDF_V5) as pdf:
        ...
        annex_page(pdf, 12)
    shutil.copy2(PDF_V5, FINAL_PDF_V5)
    return 12
...
def update_readme() -> None:
    ...
    block_v5 = """## Versión DataGastro V5 (final)
...
"""
    pattern_v5 = r"(?ms)^## Versión DataGastro V5 \(final\)\n.*?(?=^## |\Z)"
    if re.search(pattern_v5, text):
        text = re.sub(pattern_v5, block_v5.rstrip() + "\n\n", text)
    else:
        text = text.rstrip() + "\n\n" + block_v5
...
def main() -> None:
    ...
    print("=" * 72)
    print("Informe Cafecito DataGastro V5 (final)")
    ...
    print(f"  PDF: {PDF_V5}")
    print(f"  Copia final: {FINAL_PDF_V5}")
    print("=" * 72)
